...
var Enconvert = class {
  ...
  async download(url, dest) {
    ...
    await promises$1.pipeline(stream.Readable.fromWeb(webStream), fs.createWriteStream(dest));
  }
  /** Convert a `FileInput` into a normalized `{ bytes, filename, contentType }`. */
  async toFilePart(file) {
    if (typeof file === "string") {
      const bytes = new Uint8Array(await promises.readFile(file));
      const filename = path.basename(file);
      return { bytes, filename, contentType: mimeFor(filename) };
    }
    if (file instanceof Uint8Array || buffer.Buffer.isBuffer(file)) {
      return {
...
exports.Enconvert = Enconvert;
exports.EnconvertError = EnconvertError;
